![[dancing-spheres-logo.svg|Dancing Spheres Logo]]

Welcome to the D12Expeditions BETA SRD. These rules are in flux and open to comment and playtest. If you want to playtest the rules we are developing an introductory box set of character playbooks and adventures. Join us on the [D12 Expeditions Discord ](https://discord.gg/K6rUS3f8GZ) and check out our other works in playtesting.

licensed under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) ![Creative Commons](https://mirrors.creativecommons.org/presskit/icons/cc.svg) ![BY 4.0](https://mirrors.creativecommons.org/presskit/icons/by.svg)
# To use this page

Create an account, read the current SRD and contribute your comments. Your feedback will help shape the system development going forwards.

# Our Goals

The goal is to create a broad system that is simple to learn and consistent in it mechanical system so if you know one system, you are already familiar when presented with a new system. We are building a system that supports tactical combat but with robust roleplaying and non-combat support. It is pretty obvious that this is a derivation of various D20 systems, but we think our distillation is unique in two ways.

## Encourages teamwork

From out combat design to complex rolls we want players to be collaborating at every turn. They are encouraged to use their actions to set up their allies to go above and beyond what they can do on their own.

## Encourages Engagement

Too often as soon as a character no longer has the spotlight the player will step away and disengage to some extent. We hope that working together to overcome conflicts and providing reactive features that can allow characters to intervene on their allies behalf will encourage engagement.

# Table of Contents

## [[01. Playing the Game]]
Players start here to learn the core system of the game.

## [[02. Creating a Character]]
Instructions for creating characters by choosing Ancestry, Culture, Background, Archetype, choosing Stats, and equipping them.

## [[03. Running the Game]]
Basics of the game for Game Masters.

## [[04. Running Conflicts]]
Rules for when Characters come in conflict with their Foes.

## [[05. Outside of Conflicts]]
What to do when not adventuring.

## [[06. Adventuring]]

Tools for building Adventures.

## [[07. Rewards]]

Rewards for Adventures.

## [[08. Advancement]]

All the details players need to level their characters.

## [[09. Enigmas]]

Details on using magic.

## [[10. Foes]]

Creating encounters and foes.

## [[11. Items]]

Creating Alchemical and Enchanted Items.

# Appendix

## [[I. Spell Lists]]

## [[II. Item Lists]]

## [[III. Foe Lists]]